...
int RegexHandler::exitHandler( ) { return EXIT; };
...
int RegexHandler::runHandler( ) {
	...
	return SUCCESS; // error messages if any will be printed in recursive call to
					// handle
}
...
int RegexHandler::selectAttrFromWhereHandler( ) {
	...
	int ret = Frontend::select_attrlist_from_table_where(
		sourceRelName, targetRelName, attrCount, attrNames, attribute, op, value );
	if ( ret == SUCCESS ) {
		cout << "Selected successfully into " << targetRelName << endl;
	}
...
int RegexHandler::selectFromJoinHandler( ) {
	...
	int ret = Frontend::select_from_join_where(
		sourceRelOneName, sourceRelTwoName, targetRelName, joinAttributeOne, joinAttributeTwo );
	if ( ret == SUCCESS ) {
		cout << "Selected successfully into " << targetRelName << endl;
	}
...
int RegexHandler::selectAttrFromJoinHandler( ) {
	...
	int ret = Frontend::select_attrlist_from_join_where(
		sourceRelOneName, sourceRelTwoName, targetRelName, joinAttributeOne, joinAttributeTwo, attrCount, attrNames );
	if ( ret == SUCCESS ) {
...
void printHelp( ) {
	printf( "CREATE TABLE tablename(attr1_name attr1_type ,attr2_name "
			"attr2_type....); \n\t -create a relation with given attribute "
			"names\n \n" );
	...
	printf( "ALTER TABLE RENAME tablename TO new_tablename;\n\t-rename an "
			"existing relation to a given new name. \n\n" );
	printf( "ALTER TABLE RENAME tablename COLUMN column_name TO "
	...
	printf( "SELECT * FROM source_relation INTO target_relation; \n\t-creates a "
			"relation with the same attributes and records as of source "
			"relation\n\n" );
	printf( "SELECT Attribute1,Attribute2,....FROM source_relation INTO "
			"target_relation; \n\t-creates a relation with attributes specified "
			"and all records\n\n" );
	printf( "SELECT * FROM source_relation INTO target_relation WHERE attrname OP "
			"value; \n\t-retrieve records based on a condition and insert them "
			"into a target relation\n\n" );
	printf( "SELECT Attribute1,Attribute2,....FROM source_relation INTO "
			"target_relation;\n\t-creates a relation with the attributes specified "
			"and inserts those records which satisfy the given condition.\n\n" );
	printf( "SELECT * FROM source_relation1 JOIN source_relation2 INTO "
			"target_relation WHERE source_relation1.attribute1 = "
			"source_relation2.attribute2; \n\t-creates a new relation with by "
			"equi-join of both the source relations\n\n" );
	printf( "SELECT Attribute1,Attribute2,.. FROM source_relation1 JOIN "
			"source_relation2 INTO target_relation WHERE source_relation1.attribute1 "
			"= source_relation2.attribute2; \n\t-creates a new relation by equi-join "
			"of both the source relations with the attributes specified \n\n" );
